...
// Returns all contacts linked to the given contact_id
...
// Body: { user_id, contact_id_a, contact_id_b, household_name? }
export async function POST(req: Request) {
  ...
  if (!isUuid(uid) || !isUuid(a) || !isUuid(b)) {
    return NextResponse.json({ error: "Invalid IDs" }, { status: 400 });
  }
  if (a === b) return NextResponse.json({ error: "Cannot link a contact to itself" }, { status: 400 });
  ...
  // Normalize order so a < b to avoid duplicate entries
...
// Body: { link_id } OR { contact_id_a, contact_id_b }
export async function DELETE(req: Request) {
  const body = await req.json().catch(() => ({}));

  if (body.link_id && isUuid(String(body.link_id))) {
    const { error } = await supabaseAdmin.from("contact_links").delete().eq("id", body.link_id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  ...
  const a = String(body.contact_id_a || "");
  const b = String(body.contact_id_b || "");
  if (!isUuid(a) || !isUuid(b)) return NextResponse.json({ error: "Invalid IDs" }, { status: 400 });
